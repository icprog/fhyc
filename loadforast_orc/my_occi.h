...
public:
    ...
    long exec_sql(char *sqlstmt);

    long long exec_select(char *sql_string, int column_num, const char *desc);

    void report_error(OCIError *errhp);

    double GetDouble(long column,long row);

    float GetFloat(long column, long row);

    string GetStr(long column,long row);

    long GetInt(long column,long row);
