...
public:
  ...
  int GetIntDay(){return this->m_nDay;}
   ...
   string GetStrDate()const{return this->m_cDate;}
  ...
  static int CalMonthDays(string Month);
...
friend CMyDate operator +(const CMyDate& a, int b);



};

inline CMyDate operator +(const CMyDate& a, int b)
{
    return CMyDate::AddDayNum(a,b);
}
inline CMyDate operator -(const CMyDate& a, int b)
{
    return CMyDate::AddDayNum(a,-b);
