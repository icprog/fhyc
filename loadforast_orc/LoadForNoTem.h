...
{
private:
    //the span of past Days,start from: CalComDay(m_iToyearStart) - PastDaySpan,
    //end to:CMyDate::AddDayNum(m_iPastStart, ComDayNum) + PastDaySpan.
    int PastDaySpan;
    //in order to calculate Ascending Rate,the num of days is ComDayNum
    int ComDayNum;
  public:
...
public:
    ...
    void InitZeroLoad(double Energy,double MaxPower,string Type);
    void SetLoadDay(map<CMyDate,CLoadDay> &Container,string Date,double Energy,
                    double MaxPower, string Type)
    {
        map<CMyDate,CLoadDay>::iterator Iter_temp = Container.find(CMyDate(Date));
        if(Iter_temp == Container.end())
        {
            cout<<"Couldn't find the LoadDate you want to set,Date:"<<Date<<endl;
            PrintLDs(Container);
            exit(1);
        }
        Iter_temp->second.SetMaxPower(MaxPower);
        Iter_temp->second.SetEnergy(Energy);
        if("" != Type)
        {
            AddType(Date,Type);
            cout<<"Type to be set:"<<Date.c_str()<<",type:"<<Type<<endl;
            Iter_temp->second.SetType(Type);
        }
    }
    ...
    //set the last Type code,6 means Saturday,7 means Sunday,0 means workday.
    void SetLastType(const string &Date,string &Type)
    {
        CMyDate tmp(Date);
        if(7 == CMyDate::CalWeekDay(tmp))
        {
            Type[Type.length()-1]='7';
        }
        else if(6 == CMyDate::CalWeekDay(tmp))
        {
            Type[Type.length()-1]='6';
        }
        else
            Type[Type.length()-1]='0';
    }

    //add the last Type code,6 means Saturday,7 means Sunday,0 means workday.
    void AddType(string &Date,string &Type)
    {
        CMyDate tmp(Date);
        if(7 == CMyDate::CalWeekDay(tmp))
        {
